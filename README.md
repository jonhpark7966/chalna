...
|------------|-----|-----------|------------|---------|---------|------|
| 2분 | 15s | 1s | 39s | **55s** | 30 | 0 |
| 5분 | 43s | 3s | 110s | **2분 40초** | 117 | 0 |
| 10분 | 89s | 6s | 112s | **3분 30초** | 246 | 0 |
| 30분 | 242s | 17s | 431s | **11분 40초** | 722 | 3 |
| 114분 | 1977s | 102s | 1681s | **62분 42초** | 2626 | 12 |

**참고 사항**:
- **ASR**: 오디오 길이에 선형 비례 (~8초/분). 11분 이상은 10분 청크로 분할 처리
- **Alignment**: 전체 시간의 2~3%. 세그먼트당 ~0.04초
- **LLM Refinement**: 세그먼트 수에 비례 (~0.6초/세그먼트). Codex API 부하에 따라 변동 가능
...
|------|--------|-------------------|------|
| Job 1 | 18s | `0` → 완료 | 즉시 처리 |
| Job 2 | 33s | `1` → `0` → 완료 | Job 1 완료 후 처리 |
| Job 3 | 48s | `2` → `1` → `0` → 완료 | Job 2 완료 후 처리 |

- 3건 직렬 처리 확인: wall time 48s ≈ 단일 작업 16s × 3
